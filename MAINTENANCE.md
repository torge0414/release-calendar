...
GitHub Actions 的 `daily-scores.yml` 每天北京时间 18:00 运行，也可在 Actions 页面手动运行。脚本只读取现有 `mc_url` 与电影 `sid`；不会增删作品、改日期或平台。Metacritic 页面没有评分、豆瓣未开放评分或来源请求失败时，保留原分数。只有分数或豆瓣评分人数变化时才更新 JSON 并提交。多半来源请求失败时整次运行报错，避免把异常抓取当成正常结果。
...
要实际写入数据，去掉 `--dry-run`。更新后再执行 `python scripts/check_data.py`，确认差异仅涉及评分、人数和更新时间。
